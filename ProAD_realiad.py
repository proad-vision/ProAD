    ...
    for epoch in range(int(math.ceil(total_iters / len(train_dataloader)))):
        ...
        for data_batch in train_dataloader:
            ...
            if (it + 1) == total_iters:
                ...
                model_save_path = os.path.join(model_params_save_dir, "realiad_student.pth")
...
def train_segmentation_model(args, item_list_selected, device, print_fn, mvtec_item_list_all_for_train):
    ...
    params_path = os.path.join("checkpoints", "realiad_student.pth")
    ...
    for epoch in range(num_epochs_to_train):
        ...
        if (epoch + 1) % 1 == 0:
            ...
            model_save_path = os.path.join("checkpoints", "realiad_afm_upsampler.pth")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_name', type=str, default='realiad_anomaly_detection')
    args = parser.parse_args()

    item_list_all = ['audiojack', 'bottle_cap', 'button_battery', 'end_cap', 'eraser', 'fire_hood',
                 'mint', 'mounts', 'pcb', 'phone_battery', 'plastic_nut', 'plastic_plug',
                 'porcelain_doll', 'regulator', 'rolled_strip_base', 'sim_card_set', 'switch', 'tape',
                 'terminalblock', 'toothbrush', 'toy', 'toy_brick', 'transistor1', 'usb',
                 'usb_adaptor', 'u_block', 'vcpill', 'wooden_beads', 'woodstick', 'zipper']
    obj_list = ['audiojack', 'bottle_cap', 'button_battery', 'end_cap', 'eraser', 'fire_hood',
                 'mint', 'mounts', 'pcb', 'phone_battery', 'plastic_nut', 'plastic_plug',
                 'porcelain_doll', 'regulator', 'rolled_strip_base', 'sim_card_set', 'switch', 'tape',
                 'terminalblock', 'toothbrush', 'toy', 'toy_brick', 'transistor1', 'usb',
                 'usb_adaptor', 'u_block', 'vcpill', 'wooden_beads', 'woodstick']
    texture_list = ['zipper']
    ...
    student_model_path = os.path.join("checkpoints", "realiad_student.pth")
