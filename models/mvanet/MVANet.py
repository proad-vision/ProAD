# Vendored from the official MVANet implementation (MIT License):
#   https://github.com/qianyu-dlut/MVANet
#   Paper: "Multi-view Aggregation Network for Dichotomous Image Segmentation", CVPR 2024.
# Modification for ProAD: PositionEmbeddingSine is turned into a device-agnostic
# nn.Module (the original hardcodes device='cuda', which breaks CPU inference).
# Its dim_t tensor is a non-persistent buffer, so state_dict keys are unchanged.
# See ./LICENSE for the original license text.
...
class PositionEmbeddingSine(nn.Module):  # ProAD: nn.Module so device follows model.to()
    def __init__(self, num_pos_feats=64, temperature=10000, normalize=False, scale=None):
        ...
        if scale is None:
            scale = 2 * math.pi
        self.scale = scale
        # ProAD: non-persistent buffer instead of a tensor hardcoded on 'cuda',
        # so the embedding follows model.to(device) and works on CPU as well.
        self.register_buffer('dim_t', torch.arange(0, self.num_pos_feats, dtype=torch.float32), persistent=False)
