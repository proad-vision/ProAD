...
def new_evaluation_batch(model, model_2, dataloader, device, _class_=None, max_ratio=0, resize_mask=None):
    model.eval()
    model_2.eval()
    gt_list_px = []
    pr_list_px = []
    ...
    with torch.no_grad():
        for img, gt, label, img_path in dataloader:
            ...
            # starter.record()
            output = model(img)
            en, de = output[0], output[1]  # en是编码器的两个输出，de是解码器的两个输出
            en_0 = en[0]
            de_0 = de[0]  
            en_1 = en[1]
            de_1 = de[1]
            ...
            if resize_mask is not None:
                anomaly_map = F.interpolate(anomaly_map, size=resize_mask, mode='bilinear', align_corners=False)
                gt = F.interpolate(gt, size=resize_mask, mode='nearest')
                seg_out = F.interpolate(seg_out, size=resize_mask, mode='bilinear', align_corners=False)
            ...
            if max_ratio == 0:
                ...
                temp_map = anomaly_map.flatten(1)
                sp_score = torch.sort(temp_map, dim=1, descending=True)[0][:, :int(temp_map.shape[1] * max_ratio)]
                sp_score = sp_score.mean(dim=1)
            
            pr_list_sp.append(sp_score)

            anomaly_map = anomaly_map + seg_out  # 此时才加上 model_2 的结果
